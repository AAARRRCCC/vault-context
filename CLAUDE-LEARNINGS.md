...
### 2026-03-25 — WO-075: --chrome flag + browser spike

- `--chrome` flag added to `mayor-check.sh` headless invocation. Whether it connects depends on whether Chrome + the Claude in Chrome extension are running and can accept a WebSocket. In headless launchd sessions (no TTY, no GUI), the connection likely fails silently and the session proceeds without browser tools.
- `claude-in-chrome` MCP server only appears when Claude Code is started interactively with `--chrome` AND a live Chrome extension is listening. It does NOT appear in `claude -p` headless sessions without a GUI context.
- The next heartbeat run will be the real test — check `~/.local/log/mayor-check.log` for any chrome connection output after the next cycle.
- For true headless browser automation, `@playwright/mcp` remains the recommended path (no GUI dependency).
