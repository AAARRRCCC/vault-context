...
MCP servers are configured via `claude mcp add`, NOT via `settings.json`. The `settings.json` file rejects `mcpServers` as an unrecognized field.

```bash
# Add a user-scoped MCP server (available in all sessions on this machine):
...
Scope options: `user` (global, `~/.claude.json`), `local` (project-local), `project` (checked-in `.mcp.json`).

**Installed MCP servers (user scope):**
- `playwright` — `npx @playwright/mcp@latest` — 22 browser automation tools, headless Chromium. Used by tweet pipeline for reading JS-rendered linked pages.
- `basic-memory` — vault knowledge graph search
