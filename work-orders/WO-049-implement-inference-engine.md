...
> **Depends on:** WO-048 (spec — read it first)
> **Pre-read:** `work-orders/WO-048-connection-inference-spec.md` for full design rationale

---

## Objective

Create `backend/app/services/scanner/connection_inference.py` — a standalone service that takes a dict of discovered devices and a target subnet, and returns a list of connection dicts ready for `graph_builder.upsert_connection()`.

This WO implements **Strategies 2 and 3** from the spec (gateway inference and switch-aware subnet inference). Strategy 1 (LLDP) is deferred to WO-051 since it requires coordinator wiring done in WO-050.

**This WO does NOT modify scan_coordinator.py or any other existing file.** It's a new file only, testable in isolation.
...
git checkout -b plan-b/connection-inference  # or checkout if it exists
git tag pre-WO-049
```
...
Create `network-topology-mapper/backend/app/services/scanner/connection_inference.py` with the following content. Read the inline comments — they explain the "why" for each decision.
...
Worker should verify the file is syntactically valid and imports resolve:

```bash
cd ~/projects/network-topology-scanner/network-topology-mapper/backend
python3 -c "
import ast, sys
with open('app/services/scanner/connection_inference.py') as f:
    ast.parse(f.read())
print('Syntax OK')
"
```

Also verify the import chain won't blow up (this import will fail on missing deps like neo4j, but the file itself should parse):

```bash
cd ~/projects/network-topology-scanner/network-topology-mapper/backend
python3 -c "
# Just verify the module's own imports are stdlib-only
import ipaddress, logging, uuid
from datetime import datetime
from typing import Optional
print('All connection_inference imports are stdlib — no external deps needed')
"
```
...
git commit -m 'WO-049: Add connection inference engine (gateway + switch-aware strategies)'
git tag post-WO-049
```
...
- This file has **zero external dependencies** — only stdlib. It doesn't import anything from the app. That's intentional: it makes it testable in isolation and avoids import chain issues.
- The `_infer_from_lldp()` method is a stub. Don't implement it — that's WO-051.
- The `_find_best_switch()` VLAN matching is a best-effort heuristic. On Brady's home network there probably aren't managed switches, so Strategy 2 (gateway) will do the heavy lifting. Strategy 3 exists for the mock data / small-office scenario.
- Don't run `docker-compose up` or test against Neo4j. Just verify syntax and commit.
