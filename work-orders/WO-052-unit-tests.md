...
# WO-052 — Connection Inference Unit Tests

> **Plan B, WO 4 of ~6** — Worker execution task.
...
Write unit tests for `ConnectionInferenceEngine`. The module has zero app-internal imports (all stdlib), so tests run in pure Python with no Docker, Neo4j, or Redis required.
...
Create `network-topology-mapper/backend/tests/test_connection_inference.py`.

If the `tests/` directory doesn't exist, create it with an `__init__.py`.

```bash
mkdir -p network-topology-mapper/backend/tests
touch network-topology-mapper/backend/tests/__init__.py
```
...
class TestSwitchAwareInference:
    ...
    def test_basic_hierarchy(self):
        ...
        # Laptop to core switch (not directly to router)
        assert frozenset({"sw-core", "laptop"}) in edges
        # Laptop should NOT connect directly to router
    ...
    def test_access_switches(self):
        ...
        # Workstation on VLAN 10 should connect to access switch on VLAN 10
    ...
    def test_vlan_matching(self):
        ...
        edges = edge_set(conns)
        # Server on VLAN 20 should connect to sw-20, not sw-10
...
class TestDeduplication:
    ...
    def test_no_duplicate_edges(self):
        ...
        edges = edge_set(conns)
        assert len(edges) == len(conns)  # no duplicates means set size == list size
...
class TestEdgeCases:
    ...
    def test_device_with_no_ip(self):
        ...
        # Ghost device has no IP, can't be placed in subnet → no edge to it
        assert len(conns) == 0
...
pip install pytest --break-system-packages 2>/dev/null || pip3 install pytest --break-system-packages

# Run the tests
python -m pytest tests/test_connection_inference.py -v
```

All tests should pass. If any fail, fix the test (not the engine) only if the test has a bug. If the engine has a bug, note it in the result file and stop — Mayor will decide.
...
tests/test_connection_inference.py::TestSwitchAwareInference::test_access_switches PASSED
tests/test_connection_inference.py::TestSwitchAwareInference::test_vlan_matching PASSED
...
```bash
git add network-topology-mapper/backend/tests/
git commit -m 'WO-052: Unit tests for connection inference engine (20 tests)'
git tag post-WO-052
```
...
- The `sys.path.insert` hack is needed because `connection_inference.py` lives inside the `app/` package but has no app-internal imports. We're importing it directly without bootstrapping FastAPI or any dependencies.
- If Python version on Mac Mini doesn't support `list[dict]` syntax in type hints (< 3.9), the import will fail with a TypeError. The NTS repo targets 3.11+ per the tech stack, so this should be fine. If it fails, note the Python version in the result file.
- Don't install the full `requirements.txt` — you don't need Neo4j, Redis, etc. Just pytest.
- If any test fails, paste the full pytest output in the result file. Don't modify the engine without Mayor review.
