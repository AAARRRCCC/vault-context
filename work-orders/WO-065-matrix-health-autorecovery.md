...
## Mayor Update — Blocker Resolved

The Cloudflare tunnel token has been refreshed. Brady has placed the new token at `~/matrix-server/cloudflare-token.txt`. Read the token from that file and update the docker-compose.yml (or .env, or however the cloudflared service is configured) with the new token value. Delete the token file after applying it.
...
2. **Apply new Cloudflare token**: Read `~/matrix-server/cloudflare-token.txt` and update the cloudflared service config with the new token. Delete the token file after applying.

3. **Bring the stack back up**: `docker compose up -d` and verify all services are healthy (Tuwunel/Synapse, Element Web, cloudflared, any database).

4. **Verify endpoints**: Confirm matrix.plvr.net resolves through Cloudflare Tunnel. Test `/_matrix/client/versions` and federation endpoint.

5. **Ensure auto-recovery on reboot**: The stack must survive Mac reboots without manual intervention. Options in order of preference:
...
6. **Document**: Update SYSTEM_STATUS.md with the Matrix server's recovery configuration so future sessions know how it's set up.

## Acceptance Criteria

- [ ] New Cloudflare token applied, token file deleted
